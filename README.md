...
- **iPhone footage:** iPhones record HEVC (H.265) by default. When Windows cannot decode a video (HEVC without the **HEVC Video Extensions**, or 10-bit and 4:4:4 exports), it is converted to H.264 automatically on first use and the copy is reused. Recording in **Settings → Camera → Formats → Most Compatible** skips the conversion. iPhone videos also record their lens, which both workspaces read automatically. For other footage Third Person estimates the lens from the picture (MoGe-2), which keeps distances toward and away from the camera consistent whatever the phone or zoom.
...
- **First Person** animates the s&box first-person viewmodel arms by default: **Human arms** (`facepunch.v_first_person_arms_human`, five fingers) or **Citizen arms** (`facepunch.v_first_person_arms_citizen`, four fingers), picked next to the preview. The export plays directly on those models; add the package to your project to use them in game. Full-body and custom targets remain available.
