...
sealed class MocapContactTimeline : Widget
{
    ...
    public void SetMotion(MotionDocument value)
    {motion=value;Visible=value?.Contacts.Count>0;Update();}
    ...
    {
        var hits=Hits(e.LocalPosition);Cursor=CursorShape.Finger;
        ToolTip=hits.Length==0?"Click to seek. Yellow contacts need review; green are confirmed; gray are disabled.":
    ...
    {
        var hits=Hits(e.LocalPosition);var expected=motion;
        if(expected is null)return;
        if(e.LeftMouseButton)
        {
            Seek?.Invoke(Fraction(e.LocalPosition.x));
            if(e.IsDoubleClick&&hits.Length==1)Edit?.Invoke(expected,hits[0]);
            e.Accepted=true;
        }
        else if(e.RightMouseButton&&hits.Length>0)
        {
