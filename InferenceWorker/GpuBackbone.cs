...
{
    public const string BuilderVersion="vit-backbone-v1";
    ...
    readonly InferenceSession session;readonly int tokens;
    ...
    public static GpuBackbone? TryCreate(string checkpointPath,string checkpointSha256,int tokens,string cache,Action<string>? report,CancellationToken cancellation)
    {
        if(Device is not { } device)return null;
        try
        {
            var backbone=new GpuBackbone(checkpointPath,checkpointSha256,tokens,cache,device,cancellation);
            var probe=new float[tokens*Width];var random=new Random(1);for(var i=0;i<probe.Length;i++)probe[i]=(float)(random.NextDouble()-.5);
    ...
    GpuBackbone(string checkpointPath,string checkpointSha256,int tokens,string cache,(int Index,string Name) device,CancellationToken cancellation)
    {
        this.tokens=tokens;Adapter=device.Name;Directory.CreateDirectory(cache);
        var stem=$"vit-h-{checkpointSha256[..16].ToLowerInvariant()}-{tokens}t-fp16-{BuilderVersion}";var graphPath=Path.Combine(cache,stem+".onnx");
        if(!File.Exists(graphPath))Build(checkpointPath,cache,stem,tokens,cancellation);
    ...
    /// <summary>Tokens after patch embedding (and any extra tokens), row-major [tokens,1280]; returns them after the final norm.</summary>
    public float[] Run(float[] input)
    {
        if(input.Length!=tokens*Width)throw new ArgumentException("Unexpected backbone token count.");
        using var value=OrtValue.CreateTensorValueFromMemory(input,new long[]{1,tokens,Width});
        using var run=new RunOptions();
        using var outputs=session.Run(run,new[]{"tokens"},new[]{value},new[]{"features"});
        return outputs[0].GetTensorDataAsSpan<float>().ToArray();
    }
    public void Dispose()=>session.Dispose();

    static void Build(string checkpointPath,string cache,string stem,int tokens,CancellationToken cancellation)
    {
        ...
        using(var data=new FileStream(dataPartial,FileMode.Create,FileAccess.Write,FileShare.None,1<<20))
        {
            ...
            graph.Input("tokens",OnnxGraph.Float,1,tokens,Width);graph.Output("features",OnnxGraph.Float,1,tokens,Width);
            ...
            graph.Identity(Norm(x,"backbone.last_norm"),"features");
            data.Flush();
