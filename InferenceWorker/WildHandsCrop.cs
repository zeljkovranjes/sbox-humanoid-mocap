...
/// <summary>WildHands demo dataset preprocessing at full source resolution. The
/// optional outer demo rendering resize is omitted; intrinsics and boxes stay in
/// original pixels until the dataset's 224-pixel camera transform.</summary>
public static class WildHandsCrop
{
    ...
    public static Prepared Prepare(DecodedVideoFrame frame,Box? right,Box? left,Camera camera)
    {
        ...
        using var blurred=new Mat();Cv2.GaussianBlur(rgb,blurred,new Size(5,5),8);
        var size=Math.Max(frame.Width,frame.Height);var scale=224f/size;
        using var square=Crop(blurred,frame.Width/2f,frame.Height/2f,size);
        ...
        var patchCamera=new Camera(camera.Fx*scale,camera.Fy*scale,
            (camera.Cx-frame.Width/2f)*scale+112,(camera.Cy-frame.Height/2f)*scale+112,camera.Calibrated);
        (float[] Image,float[] Center,float[] Corners) Hand(Box? box)
        {
            ...
            if(box is not null)
            {
                ...
                int X(float x)=>Math.Clamp((int)(scale*(x-frame.Width/2f)+112)+1,0,223);
                int Y(float y)=>Math.Clamp((int)(scale*(y-frame.Height/2f)+112)+1,0,223);
    ...
    static Mat Crop(Mat image,float cx,float cy,float size)
    {
        ...
        var result=new Mat();
        try{Cv2.WarpAffine(image,result,affineFloat,new Size(224,224),InterpolationFlags.Cubic,BorderTypes.Constant,Scalar.Black);return result;}
